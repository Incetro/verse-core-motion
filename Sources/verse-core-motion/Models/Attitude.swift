...
public struct Attitude: Hashable {
    ...
    /// CMQuaternion instance
    public var quaternion: CMQuaternion

    // MARK: - Initializers

    /// CMAttitude initializer
    /// - Parameter attitude: CMAttitude instance
    public init(_ attitude: CMAttitude) {
        self.quaternion = attitude.quaternion
    }

    /// CMQuaternion initializer
    /// - Parameter quaternion: CMQuaternion instance
    ...
    public var pitch: Double {
        ...
        return p > 1 ? Double.pi / 2 : p < -1 ? -Double.pi / 2 : asin(p)
    }
...
extension CMQuaternion {
    ...
    func multiplied(by other: Self) -> Self {
        var result = self
        result.w = w * other.w - x * x - y * other.y - z * other.z
        result.x = w * other.x + x * w + y * other.z - z * other.y
        result.y = w * other.y - x * z + y * other.w + z * other.x
        result.z = w * other.z + x * y - y * other.x + z * other.w
        return result
    }
}
#endif
